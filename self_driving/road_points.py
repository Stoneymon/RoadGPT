    ...
    def plot_height_on_ax(self, ax):
        def _plot_dz(points, color, linewidth):
            ...
            for i in range(1, len(points)):
                d.append(d[-1] + (np.sqrt((points[i][0] - points[i-1][0]) ** 2 + (points[i][1] - points[i-1][1]) ** 2)))
                z.append(z[-1] + (points[i][2] - points[i-1][2]))
            ax.plot(d, z, color=color, linewidth=linewidth)
            return d[-1] * (15/100)


        diff = _plot_dz(self.middle, 'black', linewidth=1)
        ax.set_ylim(-diff, diff)
