...
from beamngpy.sensors import Electrics, Timer, Sensor, State, GForces, Camera
from typing import List, Tuple

VehicleStateProperties = ['timer', 'pos', 'dir', 'vel', 'steering', 'steering_input',
                          'brake', 'brake_input', 'throttle', 'throttle_input',
                          'wheelspeed', 'vel_kmh', 'fuel', 'gforce_x', 'gforce_y', 'gforce_z']
...
class VehicleStateReader:
    ...
    def update_state(self):
        ...
        gf = self.vehicle.sensors['gforces']
        vel = tuple(st['vel'])
        ...
        self.state = VehicleState(timer=self.vehicle.sensors['timer'].data['time']
                                  ...
                                  , fuel=ele.data.get('fuel_volume', None)
                                  , gforce_x=gf.data.get('gx2', None)
                                  , gforce_y=gf.data.get('gy2', None)
                                  , gforce_z=gf.data.get('gz2', None))